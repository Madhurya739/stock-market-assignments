...
class CashFlowKPIs:
    ...
    def cfo_quality_score(
        cfo_series: pd.Series, pat_series: pd.Series
    ) -> Optional[float]:
        """
...
def generate_capital_allocation_csv(
    df: pd.DataFrame, output_path: str = "output/capital_allocation.csv"
):
    """
    ...
    for _, row in df.iterrows():
        ...
        cfo_quality = CashFlowKPIs.cfo_quality_score(
            pd.Series([row["cfo"]]), pd.Series([row["pat"]])
        )
        capex_int = CashFlowKPIs.capex_intensity(row["cfi"], row["sales"])
        fcf_conv = CashFlowKPIs.fcf_conversion_rate(fcf, row["op_profit"])
        pattern = CashFlowKPIs.classify_capital_allocation(
            row["cfo"], row["cfi"], row["cff"], cfo_pat_ratio=cfo_quality
        )
